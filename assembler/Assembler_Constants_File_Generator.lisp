...
(pushnew :grimoire-emulator *features*)
...
(defmacro define-dsdl-dispatch (name arglist &body body)
  `(setf (get ',(first name) ',(second name))
         #'(lambda ,arglist
...
(defun get-dsdl-data (thing kind indicator)
  (dolist (e *dsdl-objects*)
    (when (common-lisp:and (eq (first e) thing) (eq (second e) kind))
  ...
  (with-open-file (s filename :if-does-not-exist :error)
    (let ((eof (list nil)) dispfun)
      (loop for form = (read s nil eof) until (eq form eof)
            do (if (common-lisp:and (consp form)
                             (symbolp (car form))
...
(define-dsdl-dispatch (define-structure dsdl-toplevel) (form)
  (destructuring-bind (name &rest stuff) (cdr form)
    (let ((relocatable nil)
      ...
      (add-dsdl-object name :structure (make-dsdl :name name
                                                  :base base
                                                  :size (+ point base)
...
(define-dsdl-dispatch (define-lisp-structure dsdl-toplevel) (form)
  (destructuring-bind (name &rest slot-names) (cdr form)
    (dolist (slot-name slot-names)
      (note-dsdl-reference (list name slot-name) :lisp-stucture-component))
...
(define-dsdl-dispatch (define-initial-symbols dsdl-toplevel) (form)
  (destructuring-bind (&rest symbols-list) (cdr form)
    (let ((known-pkgs '(("LISP" . "LISP")
                        ("CLOE" . "CLOE")
                        ("SYSTEM" . "SYSTEM")
                        ("SYS" . "SYSTEM")
                        ("CLOS" . "CLOS")
                        ("CLOS-INTERNALS" . "CLOSI")
                        ("ALPHAOSF1" . "ALPHAOSF1")))
          (symsetups nil)
          (fdecls nil)
          (clocs nil))
...
(defun dsdl-c-upper-name-component (x)
  (map 'string #'(lambda (c)
                   ...
                   (cond ((common-lisp:and (alpha-char-p c) (upper-case-p c)) c)
                         ((common-lisp:and (alpha-char-p c) (lower-case-p c)) c)
                         ((digit-char-p c) c)
                         ((char= c #\-) #\_)
                         ((char= c #\_) #\_)
...
(defun dsdl-c-lower-name-component (x)
  (map 'string #'(lambda (c)
                   (declare (string-char c))
                   (cond ((common-lisp:and (alpha-char-p c) (upper-case-p c)) (char-downcase c))
                         ((digit-char-p c) c)
                         ((char= c #\-) #\_)
                         ((char= c #\_) #\_)
                         (t (error "Can't put ~S in a C name component." c))))
  ...
  (let ((newtoken (cdr (assoc token *conflicting-token-alist* :test #'equal))))
    ...
    (when (> (length token) *asm-token-length-max*)
      (let* ((subtoken (subseq token 0 *asm-token-length-max*))
        ...
        (when (common-lisp:and (cdr badguys) (equal token newtoken))
...
;; This will generate aihead.lisp
...
;; This will generate aistat.lisp
