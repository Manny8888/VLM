...
(defmethod initialize-instance :after ((inst instruction) &key &allow-other-keys)
...
(defclass alpha-instruction (instruction)
  ((code :initarg :code :reader instruction-code)
   (class :initarg :class :reader instruction-class)))

(defclass memory-format-instruction (alpha-instruction)
  ())

(defclass memory-format-extended-instruction (memory-format-instruction)
  ((extended-code :initarg :extended-code :reader instruction-extended-code))
  (:default-initargs :code #x18))

(defclass memory-format-branch-instruction (memory-format-instruction)
  ((branchtype :initarg :branchtype :reader instruction-branchtype))
  (:default-initargs :code #x1A))

(defclass branch-format-instruction (alpha-instruction)
  ())

(defclass operate-format-instruction (alpha-instruction)
  ((function-code :initarg :function-code :reader instruction-function-code)))

(defclass FP-operate-format-instruction (operate-format-instruction)
  ()
  (:default-initargs :code #x17))

(defclass IEEE-FP-instruction (FP-operate-format-instruction)
  ()
  (:default-initargs :code #x16))

(defclass IEEE-FP2-instruction (IEEE-FP-instruction)
  ()
  (:default-initargs :code #x16))

(defclass VAX-FP-instruction (FP-operate-format-instruction)
  ()
  (:default-initargs :code #x15))


(defclass miscellaneous-instruction (alpha-instruction)
  ())

(defclass fetch-instruction (alpha-instruction)
  ())

(defclass PAL-instruction (alpha-instruction)
  ())

(defclass unprivileged-openVMS-PAL-instruction (PAL-instruction)
  ())

(defclass priviliged-openVMS-PAL-instruction (PAL-instruction)
  ())

(defclass unpriviliged-OSF1-PAL-instruction (PAL-instruction)
  ())

(defclass priviliged-OSF1-PAL-instruction (PAL-instruction)
  ())

(defclass required-PAL-instruction (PAL-instruction)
  ())						;+++ unfinished

(defclass reserved-PAL-instructions (PAL-instruction)
  ())

(defclass reserved-digital-instructions (PAL-instruction)
  ())
...
(loop for (name code) in '((LDG #x21) (LDQ #x29) (LDS #x22)
      ...
      do (make-instance 'memory-format-instruction
...
(loop for (name type) in '((JMP 0) (JSR 1) (RET 2) (JSR_COROUTINE 3))
      do (make-instance 'memory-format-branch-instruction
...
(loop for (name code) in '((BR #x30)  (BSR #x34))
      do (make-instance 'branch-format-instruction
...
(loop for (name code function) in '((ADDL #x10 #x00) (ADDL/V #x10 #x40) (ADDQ #x10 #x20)
                                    ...
                                    (XOR #x11 #x40))
      do (make-instance 'operate-format-instruction
                        :name name :code code :function-code function :class 'IADDLOG))

;; Class SHIFTCM:  integer shift instructions
(loop for (name code function) in '((EXTBL #x12 #x06) (EXTLH #x12 #x6A) (EXTLL #x12 #x26)
...
(loop for (name code function) in '((CMPBGE #x10 #x0F) (CMPEQ #x10 #x2D)
...
(loop for (name function) in '((CPYS #x020) (CPYSE #x022) (CPYSN #x021)  (FCMOVEQ #x02A)
                               (FCMOVGE #x02D) (FCMOVGT #x02F) (FCMOVLE #x02E) (FCMOVLT #x02C)
                               (FCMOVNE #x02B) (MF_FPCR #x025) (MT_FPCR #x024))
      do (make-instance 'FP-operate-format-instruction
                        :name name :function-code function :class 'UNKNOWN))

(loop for (name function) in '((ADDS   #x080) (ADDS/C #x000)  (ADDS/M #x040)  (ADDS/D #x0c0)
      ...
      do (make-instance 'IEEE-FP-instruction
                        :name name :function-code function :class 'FPOP))

;; 2 operand instructions handled specially
(loop for (name function) in '((CVTLQ #x010)  (CVTQL #x030) (CVTQL/SV #x530) (CVTQL/V #x130)
      ...
      do (make-instance 'IEEE-FP2-instruction
                        :name name :function-code function :class 'FPOP))

(loop for (name function) in '((DIVS   #x083) (DIVS/C #x003)  (DIVS/M #x043)  (DIVS/D #x0C3)
...
(loop for (name function) in '((DIVT   #x0A3) (DIVT/C #x023)  (DIVT/M #x063)  (DIVT/D #x0E3)
...
(loop for (name code) in '((RC #xE000) (RPCC #xC000) (MB #x4000) (RS #xF000))
      do (make-instance 'miscellaneous-instruction
                        :name name :code code :class 'LD))
...
(loop for (name code) in '((FETCH #x8000) (FETCH_M #xA000))
      do (make-instance 'fetch-instruction
                        :name name :code code :class 'LD))
...
(loop for (name code) in '((AMOVRM #x00A1) (AMOVRR #x00A0) (BPT #x0080) (BUGCHK #x0081)
      ...
      do (make-instance 'unprivileged-openVMS-PAL-instruction
...
(loop for (name code) in '((HALT #x0000) (RDPS #x0036) (RDUSP #x003A) (RDVAL #x0032)
      ...
      do (make-instance 'priviliged-OSF1-PAL-instruction
                        :name name :code code :class 'PAL))
...
(loop for (name code) in '((PAL19 #x19) (PAL1B #x1B) (PAL1D #x1D) (PAL1E #x1E) (PAL1F #x1F))
      do (make-instance 'reserved-PAL-instructions
                        :name name :code code :class 'PAL))
...
(loop for (name code) in '((OPC01 #x01) (OPC02 #x02) (OPC03 #x03) (OPC04 #x04) (OPC05 #x05)
      ...
      do (make-instance 'reserved-digital-instructions
...
(defclass alpha-register (register) ())

(defclass integer-alpha-register (alpha-register) ())

(defclass FP-alpha-register (alpha-register) ())

(defmethod initialize-instance :after ((reg register) &key &allow-other-keys)
  (with-slots (name) reg
    (setf (gethash name *register-database*) reg)))

(loop for (name code aname) in '((r0 0 $0) (r1 1 $1) (r2 2 $2) (r3 3 $3) (r4 4 $4) (r5 5 $5) (r6 6 $6) (r7 7 $7)
                                 (r8 8 $8) (r9 9 $9) (r10 10 $10) (r11 11 $11) (r12 12 $12) (r13 13 $13) (r14 14 $14)
                                 (r15 15 $15) (r16 16 $16) (r17 17 $17) (r18 18 $18) (r19 19 $19) (r20 20 $20)
                                 (r21 21 $21) (r22 22 $22) (r23 23 $23) (r24 24 $24) (r25 25 $25) (r26 26 $26)
                                 (r27 27 $27) (r28 28 $28) (r29 29 $29) (r30 30 $30) (r31 31 $31)
                                 (at 28 |$at|) (gp 29 |$gp|) (sp 30 |$sp|) (zero 31 $31))
      do (make-instance 'integer-alpha-register :name name :code code :asmname aname))
...
(defun register-number (reg)
  (cond ((numberp reg) reg)
        ...
        (:otherwise (error "~a is not a valid register designator." reg))))

(defmacro define-integer-register
    (name reg &optional (printas (intern (format nil "$~a"
                                                 (register-number reg))
                                                 (find-package :grimoire-emulator))))
  `(make-instance 'integer-alpha-register
                  :name ',name
                  :code ,(register-number reg)
                  :asmname ',printas))

(loop for (name code aname) in '((f0 0 |$f0|) (f1 1 |$f1|) (f2 2 |$f2|) (f3 3 |$f3|) (f4 4 |$f4|) (f5 5 |$f5|)
                                 (f6 6 |$f6|) (f7 7 |$f7|) (f8 8 |$f8|) (f9 9 |$f9|) (f10 10 |$f10|) (f11 11 |$f11|)
                                 (f12 12 |$f12|) (f13 13 |$f13|) (f14 14 |$f14|) (f15 15 |$f15|) (f16 16 |$f16|)
                                 (f17 17 |$f17|) (f18 18 |$f18|) (f19 19 |$f19|) (f20 20 |$f20|) (f21 21 |$f21|)
                                 (f22 22 |$f22|) (f23 23 |$f23|) (f24 24 |$f24|) (f25 25 |$f25|) (f26 26 |$f26|)
                                 (f27 27 |$f27|) (f28 28 |$f28|) (f29 29 |$f29|) (f30 30 |$f30|) (f31 31 |$f31|))
      do (make-instance 'FP-alpha-register :name name :code code :asmname aname))
...
(loop for name in '(start end mark
      ...
      do (make-instance 'pseudo-instruction :name name))
...
(defgeneric emit-operation (operation &optional destination args))

(defmethod emit-operation ((operation list) &optional (destination nil) (args nil))
  (let ((instruction (find-instruction (car operation))))
    (assert (null args))
    (emit-operation instruction destination (cdr operation))))

(defmethod emit-operation :after ((operation alpha-instruction) &optional destination args)
  (declare (ignore destination args))
  (incf *instruction-counter*))

(defmethod emit-operation :after ((operation instruction) &optional destination args)
  (declare (ignore destination args))
  (setq *last-instruction* operation))

(defmethod push-operation ((operation alpha-instruction) reads writes cycles)
...
(defmethod compute-cycle-count ((operation alpha-instruction) reads writes &optional indexreg)
  ...
  (let* ((consumer-class (instruction-class operation))
    ...
    (cond ((common-lisp:and (eql (instruction-name operation) 'BIS)
             ...
             (let ((producer (aref *previous-instructions* i)))
               (destructuring-bind (&optional producer preads pwrites pcycles) producer
                 ...
                 (let* ((producer-class (instruction-class producer))
                        ...
                        (latency (common-lisp:and pcn (nth pcn latencies)))
                   ...
                   (when latency
                     (setq cycles (max (- latency
                                          (if (common-lisp:and dual-issue-permitted dual-issue)
                                              (1- cycles-so-far)
                                              cycles-so-far))
                                       cycles))))
...
(defun dual-issue-p (op1 op2)
  (let ((op1-name (instruction-name op1))
    ...
    (flet ((dual-issue (name1 class1 name2 class2)
             (common-lisp:and (or (member class1 '(LD FBR IADDLOG SHIFTCM ICMP IMULL IMULQ))
                       ...
                       (not (or (common-lisp:and (member class1 '(LD ST JSR))
                                          (member class2 '(LD ST JSR)))
                                (common-lisp:and (member class1 '(JSR IBR FBR))
...
(defmethod emit-operation ((operation memory-format-instruction) &optional (destination nil) (args nil))
  (with-slots (name class) operation
    (destructuring-bind (reg disp index &optional comment) args
      (let* ((thereg (coerce-to-register reg))
        ...
        (multiple-value-bind (cycles dual-issue)
            (compute-cycle-count operation reads writes (common-lisp:and (eql class 'ST) indexreg))
...
(defmethod emit-operation ((operation memory-format-branch-instruction) &optional (destination nil) (args nil))
  (with-slots (name) operation
    (destructuring-bind (reg index hint &optional comment) args
      (let* ((thereg (coerce-to-register reg))
             ...
             (reads (list indexreg))
             (writes (list thereg)))
        (multiple-value-bind (cycles dual-issue) (compute-cycle-count operation reads writes)
          (format destination "~&        ~a	" (instruction-pname name))
          (format destination "~a, " thereg)
...
(defmethod emit-operation ((operation branch-format-instruction) &optional (destination nil) (args nil))
  (with-slots (name) operation
    (destructuring-bind (reg disp &optional comment) args
      (let* ((thereg (coerce-to-register reg))
        ...
        (multiple-value-bind (cycles dual-issue) (compute-cycle-count operation reads writes)
          ...
          (format destination "	#~@[ ~a~]~@[ [~a~a]~]" comment cycles dual-issue)
          (push-operation operation reads writes cycles)
          (when (common-lisp:and (eql (instruction-name operation) 'BR)
...
(defmethod emit-operation ((operation operate-format-instruction) &optional (destination nil) (args nil))
  (with-slots (name) operation
...
(defmethod emit-operation ((operation FP-operate-format-instruction) &optional (destination nil) (args nil))
  (with-slots (name) operation
    (destructuring-bind (reg1 op2 destreg &optional comment) args
      (let* ((thereg (coerce-to-register reg1))
...
(defmethod emit-operation ((operation IEEE-FP-instruction) &optional (destination nil) (args nil))
  (with-slots (name) operation
    (destructuring-bind (reg1 op2 destreg &optional comment) args
      (let* ((thereg (coerce-to-register reg1))
             (theop2 (coerce-to-register-or-literal op2))
...
(defmethod emit-operation ((operation IEEE-FP2-instruction) &optional (destination nil) (args nil))
  (with-slots (name) operation
    (destructuring-bind (reg1 op2 destreg &optional comment) args
      (let* ((thereg (coerce-to-register reg1))
             (theop2 (coerce-to-register-or-literal op2))
...
(defmethod emit-operation ((operation miscellaneous-instruction) &optional (destination nil) (args nil))
  (with-slots (name) operation
    (destructuring-bind (&optional arg comment) args
      ...
      (let ((reads (if (member name '(trapb excb)) '(:all) nil)))
        (multiple-value-bind (cycles dual-issue) (compute-cycle-count operation reads nil)
          (format destination "~&        ~a	" (instruction-pname name))
...
(defmethod emit-operation ((operation fetch-instruction) &optional (destination nil) (args nil))
  (with-slots (name class) operation
    (destructuring-bind (disp index &optional comment) args
      (let* ((indexreg (coerce-to-register index))
             (reads (list indexreg))
             (writes nil))
        (multiple-value-bind (cycles dual-issue) (compute-cycle-count operation reads writes nil)
          (format destination "~&        ~a	" (instruction-pname name))
          (format destination "~a" (coerce-to-displacement disp))
          (unless (= (register-number index) 31) (format destination "(~a)" indexreg))
          (format destination "	# ~@[~a~] ~@[[~a~a]~]" comment cycles dual-issue)
...
(defmethod emit-operation ((operation PAL-instruction) &optional (destination nil) (args nil))
  (with-slots (name) operation
...
(defmethod emit-operation ((operation pseudo-instruction) &optional (destination nil) (args nil))
  (with-slots (name) operation
    (setf (pseudo-instruction-args operation) args)
    (case name
      (start (destructuring-bind (func &optional (nargs 0)) args
               (setq *func-name* func
                     *block-name* func)
      ...
      (end (destructuring-bind (func &optional comment) args
             ...
             (setq *func-name* nil)))
      (mark (destructuring-bind (markname &optional comment) args
              (when *block-name*
              ...
              (unless (null comment) (format destination "	# ~a" comment))))
      (label (destructuring-bind (labelname &optional comment) args
               (format destination "~&.align ~D" *label-alignment*)
               (setq *instruction-counter* (logand (+ *instruction-counter* 1) -2))
               (format destination "~&~a:" labelname)
               (unless (null comment) (format destination "	# ~a" comment))))
      (unlikely-label (destructuring-bind (labelname &optional comment) args
                        ;; Unlikely labels stay unaligned
                        (format destination "~&~a:" labelname)
                        (unless (null comment) (format destination "	# ~a" comment))))
      (external-branch (destructuring-bind (labelname &optional comment) args
                         (format destination "~&	br	$31, ~A" labelname)
                         (unless (null comment) (format destination "	# ~a" comment))))
      (call-subroutine (destructuring-bind (linkage labelname &optional comment) args
                         (format destination "~&	bsr	~A, ~A"
                                 (coerce-to-register linkage)
                                 labelname)
                         (unless (null comment) (format destination "	# ~a" comment))))
      ;; ;; (comment (destructuring-bind (&optional comment) args
                 (unless (null comment) (format destination "~&/* ~a */" comment))))
      (include (destructuring-bind (includefile) args
                 (load includefile :verbose t)))
      (passthru (destructuring-bind (astring &optional comment) args
                  (format destination "~&~a" astring)
                  (unless (null comment) (format destination "	# ~a" comment))))
      (otherwise (error "Unimplemented pseudo operation ~a." name)))
    

(defgeneric coerce-to-register (register))

(defmethod coerce-to-register ((register symbol))
...
(defmethod coerce-to-register ((register cons))
  (coerce-to-register (car register)))

(defmethod coerce-to-register ((register register))
  (with-slots (asmname) register
    asmname))
...
(defgeneric coerce-to-displacement (displacement))

(defmethod coerce-to-displacement ((displacement fixnum)) displacement)

(defmethod coerce-to-displacement ((displacement symbol)) displacement)


(defun asm-header (destination sourcename)
  (format destination "~&/************************************************************************")
  (format destination "~& * WARNING: DO NOT EDIT THIS FILE.  THIS FILE WAS AUTOMATICALLY GENERATED")
  (format destination "~& * FROM ~a. ANY CHANGES MADE TO THIS FILE WILL BE LOST" sourcename)
  (format destination "~& ************************************************************************/~%~%"))

(defun asm-trailer (destination sourcename)
  (format destination "~%~%~%/* End of file automatically generated from ~a */~%" sourcename))
  ...
  (with-open-file (sfs sourcefilename :direction :input)
    (with-open-file (tfs targetname :direction :output
      ...
      (let ((*package* (find-package "ALPHA-AXP-INTERNALS"))
            (*read-base* 10)
            (*print-base* 10)
            (*previous-instructions* (make-array *n-previous-instructions* :initial-element nil))
...
(defclass alpha-instruction-block ()
  ((iblock :initform (make-array 10))))

(defmethod emit-alphabits ((destination alpha-instruction-block) bits &optional disp)
  (with-slots (iblock) destination
    (vector-push-extend bits iblock)))

(defgeneric coerce-to-register-number (register))

(defmethod coerce-to-register-number ((register symbol))
  (let ((aregister (find-register register)))
    (if (null aregister) (error "Register named ~A not found." register))
    (coerce-to-register-number aregister)))

(defmethod coerce-to-register-number ((register cons))
  (coerce-to-register-number (car register)))

(defmethod coerce-to-register-number ((register register))
  (with-slots (code) register
    code))
...
(defgeneric assemble-operation (operation &optional destination args))

(defmethod assemble-operation ((operation list) &optional (destination nil) (args nil))
  (let ((instruction (find-instruction (car operation))))
    (assert (null args))
    (assemble-operation instruction destination (cdr operation))))

(defmethod assemble-operation ((operation memory-format-instruction) &optional (destination nil) (args nil))
  (with-slots (code) operation
    (destructuring-bind (reg disp index &optional comment) args
...
(defmethod assemble-operation ((operation memory-format-branch-instruction) &optional (destination nil) (args nil))
  (with-slots (code) operation
    (destructuring-bind (reg index hint &optional comment) args
      (declare (ignore comment))
      (let* ((thereg (coerce-to-register-number reg))
...
(defmethod assemble-operation ((operation branch-format-instruction) &optional (destination nil) (args nil))
  (with-slots (code) operation
    (destructuring-bind (reg disp &optional comment) args
      (declare (ignore comment))
      (let* ((thereg (coerce-to-register-number reg))
...
(defmethod assemble-operation ((operation operate-format-instruction) &optional (destination nil) (args nil))
  (with-slots (code function-code) operation
    (destructuring-bind (reg1 op2 destreg &optional comment) args
      (declare (ignore comment))
...
(defmethod assemble-operation ((operation FP-operate-format-instruction) &optional (destination nil) (args nil))
  (with-slots (code function-code) operation
    (destructuring-bind (reg1 op2 destreg &optional comment) args
      (declare (ignore comment))
      (let* ((thereg (coerce-to-register-number reg1))
...
(defmethod assemble-operation ((operation IEEE-FP-instruction) &optional (destination nil) (args nil))
  (with-slots (code function-code) operation
    (destructuring-bind (reg1 op2 destreg &optional comment) args
      (declare (ignore comment))
      (let* ((thereg (coerce-to-register-number reg1))
...
(defmethod assemble-operation ((operation IEEE-FP2-instruction) &optional (destination nil) (args nil))
  (with-slots (code function-code) operation
    (destructuring-bind (reg1 op2 destreg &optional comment) args
      (declare (ignore comment))
      (let* ((thereg (coerce-to-register-number reg1))
                        ...
                        (dpb thereg %%alpha-inst-ra
                             (dpb theop2 %%alpha-inst-rb
                                  (dpb function-code %%alpha-inst-function
                                       (dpb thedest %%alpha-inst-rc 0)))))))
        (if destination
            (emit-alphabits destination bits))
        bits))))
...
(defmethod assemble-operation ((operation miscellaneous-instruction) &optional (destination nil) (args nil))
  (nyi operation destination args))

(defmethod assemble-operation ((operation fetch-instruction) &optional (destination nil) (args nil))
  (nyi operation destination args))

(defmethod assemble-operation ((operation PAL-instruction) &optional (destination nil) (args nil))
  (nyi operation destination args))

#+ignore
(defmethod assemble-operation ((operation pseudo-instruction) &optional (destination nil) (args nil))
(with-slots (name) operation
(setf (pseudo-instruction-args operation) args)
(ecase name
(label (destructuring-bind (labelname &optional comment) args
