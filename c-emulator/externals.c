...
int CoprocessorWrite(uint32_t operand, LispObj value) {
    int result;
    ...
    switch (operand) {
        ...
        case CoprocessorRegister_VMRegisterCommand:
            VM.CommandRegister = VMCommand(LODataUnsigned (value));
            break;

        case CoprocessorRegister_VMRegisterAddress:
            VM.AddressRegister = LODataUnsigned (value);
            break;

        case CoprocessorRegister_VMRegisterExtent:
            VM.ExtentRegister = LODataUnsigned (value);
            break;

        case CoprocessorRegister_VMRegisterAttributes:
            VM.AttributesRegister = LODataUnsigned (value);
            break;

        case CoprocessorRegister_VMRegisterDestination:
            VM.DestinationRegister = LODataUnsigned (value);
            break;
        ...
        case CoprocessorRegister_VMRegisterMaskLow:
            VM.MaskRegisterLow = LODataUnsigned (value);
            break;

        case CoprocessorRegister_VMRegisterMaskHigh:
            VM.MaskRegisterHigh = LODataUnsigned (value);
            break;
        ...
        case CoprocessorRegister_StackSwitch:
            ...
            processor->StackCacheBaseVMA = LOData (value);
            processor->StackCacheTopVMA = processor->StackCacheBaseVMA + processor->
            StackCacheSize;
        ...
        case CoprocessorRegister_SaveWorld:
            if (Type_Locative != (LOTag (value) & 0x3F))
                return (FALSE);
            VirtualMemoryWriteBlockUncached((unsigned int) processor->StackCacheBaseVMA,
                                            (LispObj *) processor->StackCacheData,
                                            ((LispObj *) processor->sp - (LispObj *) processor->StackCacheData) + 1);
            SaveWorld(LOData (value));
            break;
        ...
        case CoprocessorRegister_WaitForEvent:
            SetIntervalTimer(LOData (value));
            break;

        case CoprocessorRegister_ConsoleIO:
            DoConsoleIO((EmbConsoleChannel *) HostPointer (EmbCommAreaPtr->consoleChannel),
                        (EmbConsoleBuffer *) HostPointer (LOData(value)));
            break;

        case CoprocessorRegister_AttachDiskChannel:
            AttachDiskChannel((AttachDiskChannelRequest *) MapVirtualAddressData(LOData (value)));
            break;

        case CoprocessorRegister_GrowDiskPartition:
            GrowDiskPartition((GrowDiskPartitionRequest *) MapVirtualAddressData(LOData (value)));
            break;

        case CoprocessorRegister_DetachDiskChannel:
            DetachDiskChannel((EmbPtr) LOData (value));
            break;
...
LispObj CoprocessorRead(uint32_t operand) {
    struct tms tms;
    int64_t tps;
    clock_t mstime;
    time_t tod;
    ...
    switch (operand) {
        case CoprocessorRegister_MicrosecondClock:
            /* As is typical of Unix, the error value return by the times function
            is also a valid return value.  Until the "standard" is fixed, it
            seems riskier to check for an error than to ignore it */
...
#define USE_CPU_FOR_MICROSECOND_CLOCK
        ...
        case CoprocessorRegister_CalendarClock:
            /* As is typical of Unix, the error value return by the time function
            is also a valid return value.  Until the "standard" is fixed, it
            seems riskier to check for an error than to ignore it */
            tod = time(NULL);
            ut = gmtime(&tod);
...
void InitializeTracing(int bufferSize, unsigned int startPC, unsigned int stopPC, char *outputFile) {
    ...
    if (outputFile != NULL) {
        ...
        if (traceS == NULL) {
            vpunt(NULL, "Unable to create trace file: %s", outputFile);
        }
...
static void PrintTraceRecord(TRACERECORD *traceRecord) {
    ...
    if (traceRecord->trap_p) {
        fprintf(traceS, "*** Trap %04o @ %x.%02x.%08x, microstate %x.%02x.%08x, VMA "
                        "%x.%02x.%08x\n",
                LOData (traceRecord->trap_data_0),
