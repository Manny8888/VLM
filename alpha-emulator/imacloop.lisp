...
(defmacro iloop-decrement-tos ()
  (let ((tag 't1)                       ;just for readability.
        (data 't2)
	      (exception (gensym "iloop_decrement_tos"))
	      (notnumeric (gensym "iloop_decrement_tos"))
	      (overflow (gensym "iloop_decrement_tos")))
    `((EXTLL arg6 4 ,tag)
      (LDQ arg2 CACHELINE_ANNOTATION (iCP))
      (EXTLL arg6 0 ,data)
...
(defmacro iloop-increment-tos-less-than ()
  (let ((tag 't1)                       ;just for readability.
        (data 't2)
	      (tag2 't3)
	      (data2 't4)
	      (exception1 (gensym "iloop_increment_tos_less_than"))
	      (exception2 (gensym "iloop_increment_tos_less_than"))
	      (overflow (gensym "iloop_increment_tos_less_than"))
	      (notnumeric (gensym "iloop_increment_tos_less_than")))
    `((EXTLL arg6 4 ,tag)
      (LDQ arg2 CACHELINE_ANNOTATION (iCP))
      (EXTLL arg6 0 ,data)
