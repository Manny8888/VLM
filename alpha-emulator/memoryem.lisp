...
(define-constant %memory-action-indirect 1)
(define-constant %memory-action-monitor-trap 2)
(define-constant %memory-action-transport 4)
(define-constant %memory-action-trap 10)
(define-constant %memory-action-transform 20)
(define-constant %memory-action-binding-trap 40)
...
(defun initialize-memory-actions ()
  (let ((pointer-data-types
	       (list
	        ...
	        dtp-complex dtp-spare-number dtp-instance
	        dtp-list-instance dtp-array-instance
	        dtp-string-instance dtp-nil dtp-list
	        dtp-array dtp-string dtp-symbol
	        dtp-locative dtp-lexical-closure
	        dtp-dynamic-closure dtp-compiled-function
	        dtp-generic-function dtp-spare-pointer-1
	        dtp-spare-pointer-2 dtp-bound-location
	        dtp-logic-variable dtp-even-pc dtp-odd-pc
	      ...
	      (immediate-data-types
	       (list
	        dtp-fixnum dtp-small-ratio
	        dtp-single-float dtp-physical-address
	        dtp-spare-immediate-1 dtp-character
	        dtp-packed-instruction-60
	        dtp-packed-instruction-61 dtp-packed-instruction-62
...
(defun memory-read-internal (vma tag data cycle temp temp2 temp3
  ...
  (let* ((cycle-number (case cycle
         ...
         (canindirect (not (member cycle '(processorstate_scavenge
	       ...
	       (top (gensym "vma_memory_read"))
	       (wasincache (gensym "vma_memory_read"))
	       (incache (gensym "vma_memory_read"))
	       (notindirect (gensym "vma_memory_read"))
	       (decodeaction (gensym "vma_memory_read"))
	       (decodecommontail (if #-memory-inline inlinep #+memory-inline nil
			                         (intern (concatenate 'string (string *function-being-processed*)
						                                        "DECODE"))
			                         (gensym "vma_memory_read")
                               ))
	       (doaction (gensym "vma_memory_read"))
	       (checklookup (if canlookup (gensym "vma_memory_read") doaction))
	       (checktransform (if cantransform (gensym "vma_memory_read") checklookup))
	       (checkindirect (if canindirect (gensym "vma_memory_read") checktransform))
	       (dbcachemiss (gensym "vma_memory_read"))
	       (done (or done-label (gensym "vma_memory_read")))
	       ;; readability
	       (temp1 temp)
...
(defmacro memory-write (vma tag data cycle temp temp2 temp3 temp4
  ...
  (let ((done (or done-label (gensym "vma_memory_write")))
	      (incache (gensym "vma_memory_write")))
    (unless *cant-be-in-cache-p*
      (push
	     `((label ,incache)
	       ,@(if temp5
