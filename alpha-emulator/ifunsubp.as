...
PREPARE - EXCEPTION;
can;
't be used as the opcode is variable,
;so;
we;
expand;
it;
...
;+++massive;
scheduler;
problems(e.g., sleeping;
indefinitely;
).
(LDA;
t1 | CoprocessorRegisterMicrosecondClock | (zero);
)
(SUBL;
arg1;
t1;
t2;
"Zero if Microsecond Clock";
)
(BNE;
t2;
cpreadnormal;
"J. if not read coprocessor clock";
)
(LDQ;
t5;
PROCESSORSTATE_PREVIOUSRCPP(ivory);
)
(RPCC;
t3;
...
(BEQ;
t5;
cpreadnormalresetclock;
"J. if decache request (rpcc set to zero)";
)
(ADDQ;
t3;
t6;
t6;
"Construct cycle count from two halves";
)
(LDQ;
t10;
PROCESSORSTATE_MSCMULTIPLIER(ivory);
"Get the cycle to internal units multiplier";
)
(SRL;
t6;
32;
...
(LDQ;
t9;
PROCESSORSTATE_MSCLOCKCACHE(ivory);
)
(ADDQ;
t5;
t7;
...
32;
t7;
"Previous number of ticks";
)
(SUBQ;
t6;
t7;
t8;
"Number of clocks passed since previous";
)
(BLT;
t8;
cpreadnormalresetclock;
"J. if counter wrapped.";
)
(MULQ;
t8;
t10;
t11;
"Number of internal units since last clock read";
)
(ADDQ;
t11;
t9;
...
(SRL;
t9 | MSclockUnitsToMSShift | t12;
"Convert internal units to microseconds.";
)
(stack - push - fixnum;
t12;
t1;
"Push the microsecond clock";
)
(ContinueToNextInstruction)
;(label;
cpreadnormalresetclock;
...
(BIS;
zero | TypeEvenPC | arg3;
"Treat it as an even PC";
)
(convert - continuation - to - pc;
arg3;
arg2;
t1;
t2;
