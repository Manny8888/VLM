...
this is;
:
operand - from - stack - immediate, but;
we;
can;
save;
some
...
From;
IFUNLOOP.AS
(define-instruction |DoBranchTrue|; :10-bit-signed-immediate (;:own-immediate; needs-tos; t;)
(ibranchcond;
nil;
t;
t;
...
-pop
(define-instruction |DoBranchFalse|; :10-bit-signed-immediate (;:own-immediate; needs-tos; t;)
(ibranchcond;
t;
t;
t;
...
branching;
to;
StackCacheUnderflowCheck, which;
goes
;to;
NextInstruction;
after;
dealing;
with underflow.In the;
for-return
    ;
case,
this;
...
:
own - immediate.
(define-instruction |DoReturnSingle|; :10-bit-immediate (;:own-immediate; needs-tos; t;)
...
trampoline is near;
it;
's popular callee so you gan get to
;
the;
PullApplyArgs;
tail;
...
;W - M - M - R;
for VMAinStackCache, which is used several;
times
...
From;
IFUNPRED.AS
;DoEqNoPop is handled;
here, too;
...
Note;
the | DoEqIM | is in IFUNCOM2.AS(yeah, it;
's wierd)
;Really;
operand - from - stack - immediate, but;
save;
...
own;
operand
(define-instruction |DoEq|; :operand-from-stack (;:own-immediate; needs-tos; t;)
...
(define-instruction |DoAref1|; :operand-from-stack-immediate (;:own-immediate; needs-tos; t;)
...
own;
operand
(define-instruction |DoPointerPlus|; :operand-from-stack (;:own-immediate; needs-tos; t;)
    (GetNextPCandCP)
    (stack-read-data; arg1; t2; "Get the data of op2"; :signed; tos-valid; t;)
inline(stack - read - data;
iSP;
t1;
"Get the data of op1";
:
...
inline(CheckDataType;
arg3 | TypeFixnum | LdbException;
t8;
)
(TagType; arg3; t8;)
...
From;
IFUNSUBP.AS
;DoMemoryReadAddress is handled;
here, too;
...
...
Takes;
generic function tag
/
data in ARG1 / t1;
and;
instance;
tag / data in ARG3 / ARG4.
;Returns;
mask;
data in t2, table;
data in t3, parameter;
tag / data in T6 / T7,
;and;
method;
tag / data in T4 / arg3.Clobbers;
T1;
through;
T5, and;
T10.
...
well;
!Don;
't change these memo registers without also fixing
;the;
call;
to;
USING - MULTIPLE - MEMORY - READS in | LookupHandlerMemoryRead |
(;with-multiple-memory-reads (t9 t10; t11; t12;)
...
it;
!We;
're clobbering ARG3/ARG4 to save some cycles!
(lookup-handler
;looks;
bad, but;
we;
...
temps;
aren;
't really arguments, but they are smashed
...
;(above).Note;
that;
with-multiple - memory - reads really;
should;
be
;called;
...
(define-instruction |DoStartCall|; :operand-from-stack ()
    (;with-multiple-memory-reads (t9 t10; t11; t12;)
      (stack-read2; arg1; arg5; signed; t;)
