...
(defmacro lc (str)
"converts a string to lower case, unless it is a number"
  `(if (numberp ,str)
     ,str
    (string-downcase ,str)))

(defmacro defsubst (name arglist &body body)
"defsubst is a direct replacement for defun which imposes the function to be inlined."
   `(progn
      (declaim (inline ,name))
...
"Return list of strings separated by one empty space"
  ...
  (let ((sym-name (if (symbolp str) (symbol-name str) nil)))
    (if (search "+" sym-name)
	      ...
	      (let* ((split-list (split-by-one-plus sym-name))
	        ...
	        (setf str (concatenate 'string fixed-arg "+" last-part))
...
(defun add-global-label-symbol (sym)
  (setf global-labels (append global-labels (list sym))))
...
(defun make-call-label ()
  (setf call-label-count (+ 1 call-label-count))
  ...
  (let ((cmd (car form))
	   ...
	   (if (numberp arg2)
	       ...
	       (multiple-value-bind (ptr member offset) (decompose-args arg3 arg2)
		       (if (numberp offset)
		           ...
		           (setf offset (format nil "~A/8" offset)))
		       (cond
		         ((eq offset 0)
		          ;; hack! two even!
    ...
    ;;-------------------------------------
     ...
     ;;	 (format t "passthru: arg1 ~S form ~S~%" arg1 form)
	   (if (or (equal (subseq arg1 0 5) "#ifde")
		         (equal (subseq arg1 0 5) "#ifnd")
		         (equal (subseq arg1 0 4) "#end"))
	       (format destination "~A~%" arg1)))
...
(defun create-output-files ()
  (let ((file-count 0))
    ...
    (progn
      ...
      (let ((outputfilename (format nil "output~D.c" file-count))
...
;;(process-asm-source "../alpha-emulator/ifunhead.as" "testoutput1")
