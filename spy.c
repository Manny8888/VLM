...
/* Divine a port number to use.  Try to use 2900 + xxx where xxx is
   128.81.41.xxx, if we're coming in from SLIP or something we're on a
   different subnet, so we fudge. */
...
static void segv_handler(int number) { _longjmp(trap_environment, -1); }
