...
static void CreateWorldFile ( World *world )
{
    ...
    if ( ( world->fd = open ( world->pathname, O_WRONLY | O_CREAT | O_TRUNC, S_IRUSR | S_IWUSR | S_IRGRP | S_IROTH ) ) < 
0 )
    {
...
static void FindParentWorlds ( World *world, char *worldSearchPath )
{
    ...
    while ( childWorld->sysoutGeneration )
    {
        for ( i = 0; i < nWorlds; i++ )
        {
            if ( worlds[i] && ( worlds[i]->sysoutGeneration == ( childWorld->sysoutGeneration - 1 ) )
...
static void MergeParentLoadMap ( World *world )
{
    ...
    MergeAMap ( world->nUnwiredMapEntries, world->unwiredMapEntries, world->parentWorld->nMergedUnwiredMapEntries,
                world->parentWorld->mergedUnwiredMapEntries, &world->nMergedUnwiredMapEntries, 
...
static void DumpMap ( LoadMapEntry *map, int count, char *type )
{
    ...
    for ( i = 0; i < count; i++ )
        printf ( "%d: opcode %d, address %08x, count %d, data %09lx, world %p\n", i, map[i].op.opcode, map[i].address,
                 map[i].op.count, map[i].data, map[i].world );
    printf ( "\n" );
}
#endif
...
{
    LoadMapEntry *new;
    Integer pageSizeQs = ( VLMWorldFormat == originalWorld->format ) ? VLMPageSizeQs : IvoryPageSizeQs, oldAddress, 
slop;
        ...
        else
        {
            ...
            if ( background[bIndex].address < ( foreground[fIndex].address + foreground[fIndex].op.count ) )
            {
                        ...
                        > ( foreground[fIndex].address + foreground[fIndex].op.count ) )
                {
                    ...
                    WriteLispObjData ( & ( background[bIndex].data ),
                                       LispObjData ( background[bIndex].data ) + ( background[bIndex].address - 
oldAddress ) / pageSizeQs );
                }
                else
                    /* Here iff the current background entry overlaps the
...
VLMBlocksPerDataPage;
    ...
    VirtualMemoryRead ( SystemCommSlotAddress ( sysoutTimestamp1 ), q );
    WriteIvoryWorldFileNextQ ( world, *q );
    VirtualMemoryRead ( SystemCommSlotAddress ( sysoutTimestamp2 ), q );
    WriteIvoryWorldFileNextQ ( world, *q );
    VirtualMemoryRead ( SystemCommSlotAddress ( sysoutParentTimestamp1 ), q );
    WriteIvoryWorldFileNextQ ( world, *q );
    VirtualMemoryRead ( SystemCommSlotAddress ( sysoutParentTimestamp2 ), q );
    WriteIvoryWorldFileNextQ ( world, *q );
    ...
    {
        mapEntry = &world->wiredMapEntries[i];
        q = MakeLispObj ( Type_Locative, mapEntry->address );
        WriteIvoryWorldFileNextQ ( world, *q );
        q = MakeLispObj ( Type_Fixnum, * ( Integer * ) &mapEntry->op );
        WriteIvoryWorldFileNextQ ( world, *q );
...
static void WriteVLMWorldFilePages ( World *world )
{
    ...
    for ( i = 0; i < world->nWiredMapEntries; i++ )
    {
        ...
        while ( vma < finalVMA )
        {
            ...
            if ( VMAccessFault ( attr ) || VMWriteFault ( attr ) || ( VMTransportFault ( attr ) && !VMTransportDisable ( 
attr ) ) )
        ...
        fprintf ( stderr, "Writing %d words from VMA %x ... words ... ", wordCount, mapEntry->address );
        fflush ( stderr );
#endif
...
static void ByteSwapOneWorld ( World *world )
{
    ...
    dataEnd
        = ( world->vlmDataPageBase > world->vlmTagsPageBase ) ? worldStat.st_size : VLMBlockSize * 
world->vlmTagsPageBase;
#else
