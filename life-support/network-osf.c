    ...
    for (i = 0; i < MaxNetworkInterfaces; i++)
        if (config->interfaces[i].present)
            InitializeNetChannel(
                &config->interfaces[i], i, &localHostAddress);
...
/*#define NOROOT*/
#ifndef NOROOT
    p->fd = pfopen(
        (0 == interface->device[0]) ? NULL : interface->device, O_RDWR);
    if (-1 == p->fd)
        vpunt(NULL, "Unable to open VLM network interface #%d using %s",
            unitNumber,
            (0 == interface->device[0]) ? "the default channel"
                                        : interface->device);

    if (-1 == ioctl(p->fd, EIOCIFNAME, &hardwareInterface))
        vpunt(NULL,
            "Unable to determine hardware interface name for VLM network "
            "interface #%d",
            unitNumber);
#endif
    p->name0 = p->name1 = 0;
    memcpy(
        (char *)&p->name0, hardwareInterface.ifr_name, 2 * sizeof(EmbWord));
    ...
    if (-1 == ioctl(p->fd, EIOCDEVP, &deviceParms))
        vpunt(NULL,
            "Unable to determine hardware interface address for VLM network "
            "interface #%d",
            unitNumber);
#endif
    p->hardwareAddressHigh = p->hardwareAddressLow = 0;
    memcpy((char *)&p->hardwareAddressHigh, deviceParms.end_addr,
        deviceParms.end_addr_len);

#ifndef NOROOT
    x = 1;
    if (-1 == ioctl(p->fd, EIOCALLOWPROMISC, &x))
        vpunt(NULL,
            "Unable to set ALLOWPROMISC for VLM network interface #%d",
            unitNumber);

    x = 1;
    if (-1 == ioctl(p->fd, EIOCALLOWCOPYALL, &x))
        vpunt(NULL,
            "Unable to set ALLOWCOPYALL for VLM network interface #%d",
            unitNumber);
    ...
    *fp++ = ENF_OR;
    ...
    for (interface = interface, firstInterface = TRUE; interface != NULL;
        ...
        if (interface->device[0])
            sprintf(
                addressAsString, "%s%s:", addressAsString, interface->device);
...
static void show_packet(char *who, unsigned char *pkt)
{
    ...
    if (p[12] == 0x08 && p[13] == 0) {
        ...
        printf("%s %u.%u.%u.%u:%u -> %u.%u.%u.%u:%u %s\n", who,
            p[14 + 20 - 8], p[14 + 20 - 7], p[14 + 20 - 6], p[14 + 20 - 5],
            sp, p[14 + 20 - 4], p[14 + 20 - 3], p[14 + 20 - 2],
            p[14 + 20 - 1], dp, prot);
    }
    if (p[12] == 0x08 && p[13] == 6) {
        printf(".");
...
static void NetworkChannelTransmitter(EmbNetChannel *pNetChannel)
{
    ...
    if (0)
        printf("NetworkChannelTransmitter()\n");
    while (EmbQueueFilled(transmitQueue)) {
        if (0 == EmbQueueSpace(returnQueue)) {
            /* Can't do I/O now because we can't return the buffer -- Try
             * again later */
        ...
        if (netPacketPtr != NullEmbPtr) {
            if (netChannel->status & EmbNetStatusHostReady) {
                ...
                actualBytes
                    = write(netChannel->fd, &netPacket->data[0], nBytes);
#else
                actualBytes = 0;
#endif
