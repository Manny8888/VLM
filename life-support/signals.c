...
/* Called by the emulator to wait until life support detects an event which
   requires a response by the emulator.  This mechanism is used in Genera's
   idle process to reduce the amount of host CPU time consumed by the VLM */
...
void WaitForLifeSupport()
{
    ...
    /* The emulator recognizes interrupts from Life Support only after branch
       and jump instructions (i.e., instructions which change the PC).  If we
       allowed the emulator to wait for Life Support while there are pending
       signals, we could hang. EXPLAIN FEP MODE ... (Consider the case of the
       interval timer firing before the Idle process actually waits for it.
       The clock signal would be pending but might not actually interrupt the
    ...
    if (EmbCommAreaPtr->host_to_guest_signals && ((processor->control >> 30) & TrapMode_FEP) != TrapMode_FEP)
        ...
        pthread_cleanup_push((pthread_cleanuproutine_t)pthread_mutex_unlock, (void *)&EmbCommAreaPtr->wakeupLock);
...
/* Send a signal to the VLM -- The emulator provides us with a function to
   send it an interrupt just as we provide it a function to send us an
   interrupt.  The emulator's
   function ensures that the interrupt is delivered to the VLM at an
   appropriate time */
...
/* Called by a signal handler when it can't handle the signal now and wishes
   to try later -- The polling thread checks for said handlers each "clock
   tick" */
...
void RemoveSignalHandler(SignalNumber signal)
{
    ...
    /* Leave a last interrupt pending so that the signal handler thread can
       notice that it's been cancelled and clean up gracefully */
...
/* Top level function for a signal handler thread -- We wait for our signal to
   be present and call the real handler function */
