...
void AttachDiskChannel(AttachDiskChannelRequest *pRequest)
{
    ...
    if (Type_String != *MapVirtualAddressTag((Integer)((Integer *)&request->filename - MapVirtualAddressData(0)))) {
    ...
    if (fstat(diskState->fd, &fileStatus)) {
        verror("AttachDiskChannel", "Unable to determine size of disk partition %s", filename);
        request->result = errno;
        close(diskState->fd);
        return;
    }
    ...
    diskState->command_queue_ptr->signal = InstallSignalHandler((ProcPtrV)&DiskLife, (PtrV)diskChannel, FALSE);
...
void GrowDiskPartition(GrowDiskPartitionRequest *pRequest)
{
    ...
    if (-1 == diskState->fd) {
        verror("GrowDiskPartition", "There is no disk partition attached to channel #%d", diskChannel->unit);
        request->result = EINVAL;
        return;
    }
    if (fstat(diskState->fd, &fileStatus)) {
...
void ResetDiskChannel(EmbChannel *channel)
{
    ...
    if (GuestPointer(diskChannel) > EmbCommAreaPtr->host_buffer_start + EmbCommAreaPtr->host_buffer_size) {
