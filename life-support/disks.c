...
void AttachDiskChannel(AttachDiskChannelRequest *pRequest)
{
    ...
    memcpy(
        filename, MapVirtualAddressData(request->filename + 1), filenameSize);
    ...
    if (-1 == diskState->fd) {
        verror("AttachDiskChannel", "Unable to open disk partition %s",
            filename);
    ...
    if (fstat(diskState->fd, &fileStatus)) {
        verror("AttachDiskChannel",
            "Unable to determine size of disk partition %s", filename);
        request->result = errno;
        close(diskState->fd);
        return;
    }
    ...
    diskChannel->next
        = EmbCommAreaPtr->channel_table; /* Link into the channel list */
    EmbCommAreaPtr->channel_table = GuestPointer(diskChannel);

    diskState->command_queue_ptr->signal
        = InstallSignalHandler((ProcPtrV)&DiskLife, (PtrV)diskChannel, FALSE);
...
void GrowDiskPartition(GrowDiskPartitionRequest *pRequest)
{
    ...
    if (-1 == diskState->fd) {
        verror("GrowDiskPartition",
            "There is no disk partition attached to channel #%d",
            diskChannel->unit);
        request->result = EINVAL;
        return;
    }
    if (fstat(diskState->fd, &fileStatus)) {
        verror("GrowDiskPartition",
            "Unable to determine size of disk partition attached to channel "
            "#%d",
            diskChannel->unit);
        request->result = errno;
        return;
    ...
    if (request->newLength > fileStatus.st_size) {
        if (ftruncate(diskState->fd, (off_t)request->newLength)) {
            verror("GrowDiskPartition",
                "Unable to set size of disk partition attached to channel "
                "#%d to %d bytes",
...
/* Detach the given disk channel from its disk partition (nee, file) and
   remove it from the linked list of active embedded channels */
...
static void DiskLife(EmbDiskChannel *diskChannel)
{
    ...
    while (EmbQueueFilled(commandQueue)) {
        if (EmbCommAreaPtr->inhibitDisk
            || (0 == EmbQueueSpace(statusQueue))) {
            /* Can't do I/O now -- Ask to be invoked again on the next "clock
             * tick" */
        ...
        if (commandPtr) {
            ...
            case ReadCmd:
                ...
                if (diskState->error_pending)
                    ...
                    command->error_code
                        = DoDiskIO(diskChannel, diskState, command);
                    if (command->error_code) {
                        command->status = LostStatus;
                        diskState->error_pending
                            = TRUE; /* Flush until reset */
                    } else
    ...
    EmbDiskQueueElement *command)
{
    ...
    while (nAddresses > 0) {
        ...
        for (i = 0; i < nVectors; i++, addressPair++, nAddresses--) {
            diskState->iovs[i].iov_base
                = (caddr_t)HostPointer(addressPair->address);
            diskState->iovs[i].iov_len
                = addressPair->n_words * sizeof(EmbWord);
...
void ResetDiskChannel(EmbChannel *channel)
{
    ...
    if (GuestPointer(diskChannel) > EmbCommAreaPtr->host_buffer_start
        ...
        /* If Lisp created the disk channel, we must close the attached file
           now before Lisp discards the disk channel as Lisp won't first ask
           to detach it */
        if (diskState->fd != -1) {
            close(diskState->fd);
            diskState->fd = -1;
