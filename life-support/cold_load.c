...
/* These are closely related to pixels_per_run_light as computed in
   alloc_screen_array, but any values smaller than the below (as tend to be
   computed) look crappy. */
...
static int setup_x_io_error_handler() { return (_setjmp(x_io_error)); }
        ...
        if ((XK_a <= keysym) && (keysym <= XK_z)) {
            ...
            if (event.xkey.state & ShiftMask) {
                if (XK_KP_Enter == keysym) {
                    /* Special case Shift-Keypad-Enter: Generate Return
                     * instead of End */
                    key = SK_Return;
                } else {
    ...
    if (keycode1 != 0 || keycode2 != 0) {
        ...
        if (keycode1 != 0) {
            ...
            /* Linux assigns a modifier mapping to the Num_Lock keysym but, on
               the Apple Pro keyboard, that keysym maps to the key labelled
               "clear" so it's safe to remove the modifier mapping to make
        ...
        *hyper_code = XKeysymToKeycode(display, XK_Left);
        ...
        /* Special knowledge -- DEC's LK401-AA has two Multi-Key keys labelled
           ...
           for the righthand Compose Character. The lefthand Compose
           Character's key code is four greater than the righthand key. */
...
void InitializeColdLoadChannel(VLMConfig *config)
{
    ...
    if (-1 == p->fd) {
        verror("Cold Load", NULL);
        vwarn("Cold Load",
            "Will wait for X server but cold load may not function "
            "properly.");
    } else {
        setup_x_io_error_handler();
    }
