...
void ProcessResetRequest()
{
    ...
    case FEPResetRequest:
        /* The IFEP has started running and needs a partial reset of the
         * communications area */
        ResetCommArea(FALSE);
        break;

    case LispResetRequest:
        /* Lisp has started running and needs a complete reset of the
         * communications area */
        ResetCommArea(TRUE);
    ...
    default:
        /* Silently ignore any requests which are either not supported or not
         * recognized */
        break;
    }
...
/* Top level function of the polling thread -- Does all its work under
   protection of Life Support's global signal lock as the main thrust of
   polling is to periodically wakeup all signal handlers in case a "hardware
   interrupt" from the VLM was lost. */
...
/* Top level function of the interval timer thread -- Sleeps for the interval
   requested by the emulator thread and then sends a clock interrupt.  As the
   emulator (nee, Lisp) will frequently need to reprogram the timer while
   we're waiting, we actually wait for a signal with a timeout.  The emulator
   raises the signal to indicate that it's reprogramming the interval timer.
...
/* Reprogram the interval timer -- Called from the emulator thread in response
   to writing a value to the WaitForNextEvent coprocessor register.  We simply
   set the relative time for the next interrupt and signal the interval timer
   thread.  The interval timer thread will wakeup, note that there's a new
   timeout, and go back to sleep with the new timeout. */
