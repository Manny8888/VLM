...
/* Process the individual parts of a connection request -- When opening a
   display, Lisp will send a connection setup request.  Because X doesn't
   permit said request to be issued twice, we intercept it and return the data
   that it would have returned as derived from internal data structures.  A
