/* The BootROM Communications and Data areas -- See SYS:I-SYS;SYSDF1 for
 * details */
...
#define BootCommSlotAddress(slot)                                            \
    ((ptrdiff_t)BootCommAreaAddress                                          \
        + offsetof(BootCommArea, slot) / sizeof(EmbWord))
...
#define WriteBootCommSlot(slot, datum, tag)                                  \
    VirtualMemoryWrite(                                                      \
        BootCommSlotAddress(slot), MakeLispObj((Tag)tag, (Integer)datum))
#endif
...
typedef struct {
    EmbWord
        embCommArea; /* Ivory address of the embedded communications area */
...
#define BootDataSlotAddress(slot)                                            \
    ((ptrdiff_t)BootDataAreaAddress                                          \
        + offsetof(BootDataArea, slot) / sizeof(EmbWord)                     \
        + BootDataAreaOffset)
...
#define WriteBootDataSlot(slot, datum, tag)                                  \
    VirtualMemoryWrite(                                                      \
        BootDataSlotAddress(slot), MakeLispObj((Tag)tag, (Integer)datum))
#endif
