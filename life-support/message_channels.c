...
#define SubtypeData(mc) (PtrV)(((uint64_t)mc->subtypeData0 << 32) | mc->subtypeData1)
...
void InitializeMessageChannels(VLMConfig *config)
{
    ...
    EmbMessageChannel *p = (EmbMessageChannel *)HostPointer(cp);
    EmbCommandChannel *cc;
    ...
    cc->guestToHostQueue->signal = InstallSignalHandler((ProcPtrV)&ExecuteGuestCommands, (PtrV)cc, FALSE);
...
void ExecuteGuestCommands(EmbCommandChannel *commandChannel)
{
    ...
    EmbQueue *resultsQueue = commandChannel->guestToHostReturnQueue;
    EmbPtr commandPtr;
    EmbCommandBuffer *command;
        ...
        if (commandPtr) {
            ...
            case EmbCommandBufferStartMBIN:
                ...
                if (NULL == mbinSubChannel) {
                    ...
                    mbinSubChannel->hostToGuestQueue = (EmbQueue *)HostPointer(mbinChannel->hostToGuestQueue);
                    ...
                    mbinSubChannel->guestToHostQueue->signal
                        = InstallSignalHandler((ProcPtrV)&SendMBINBuffers, (PtrV)mbinSubChannel, FALSE);
...
{
    EmbMessageChannel *messageChannel = (EmbMessageChannel *)channel;
    EmbMessageSubtypeData *subtypeData;
...
{
    /* Command and MBIN message channels don't have any state subject to clean
       up other than their signal handlers which have already been addressed
       by TerminateSignalHandlers */
}
