...
typedef struct {
    ...
    ProcPtrV handlerFunction; /* The function which actually handles the
                                 signal ... */
...
typedef struct {
    /* Overall Information */
    EmbWord identifier; /* char[4] 'EMBD' verifies this is the communication
                           area */
    EmbWord version; /* 1 identifies the communication architecture version */
    EmbWord system_type; /* SystemTypeXXX identifies the host & guest systems
                            jointly */

    EmbWord number_of_slots; /* Number of guest-visible 32-bit words in this
                                structure */
    EmbWord comm_memory_size; /* Total number of 32-bit words in communication
                                 memory */

    /* Version Numbers of various software entities, in case anyone cares */
    struct { /* Version number of this program ... */
    ...
    struct { /* Version number of the OSF/1 operating system ... */
#if BYTE_ORDER == LITTLE_ENDIAN
    ...
    /* Memory Allocation */
    EmbPtr guest_buffer_start; /* Portion of communication memory allocated by
                                  guest */
    ...
    EmbPtr bad_memory_map; /* MacIvory: Pointer to map of holes in Ivory
                              memory */
    EmbWord bad_memory_map_size; /* MacIvory: Number of 64-bit entries in that
                                    map */

    /* The remaining guest visible slots aren't in their proper logical
       position in
    ...
    struct {
#if BYTE_ORDER == LITTLE_ENDIAN
        uEmbWord status : 8; /* Guest status as determined by its FEP (see
                                below) */
        ...
        uEmbWord lisp_is_loaded : 1; /* 1 => the FEP has determined that Lisp
                                        is loaded */
        uEmbWord : 20;
#else
        uEmbWord : 20;
    ...
    EmbWord restart_applications; /* Set by guest to request host restart its
                                     applications */
    EmbWord signal_interrupt_vector; /* (Unused) Tells guest more precisely
                                        how to interrupt host */
    ...
    /* To add slots to the shared portion of the communications area, add the
       slots before the pad0 field and decrease the size of the pad1 field by
       the number of slots added. As long as spare slots remain, you won't
       have to recompile all of the host software; just those files which you
       modify to use the new slots.

       However, if you do use up all the remaining pad slots, instead of
       leaving no padding, leave a minimum of 16 pad slots (one in pad0, 15 in
       pad1).  You must also recompile all of the host software.

       Be sure that any changes you make to the shared portion of the
       communications area are reflected in the sources of all host systems
       (MacIvory, Solstice, VLM) as well as by appropriate patches in Lisp.
    */
    ...
    EmbWord resetRequestCount; /* # of times that Lisp (not IFEP) requested a
                                  reset */
    EmbWord restartApplicationsCount; /* # of times we've been requested to
                                         restart */
    ...
    /* Add new slots at the end of this structure as multiple programs use
     * this data */
} EmbCommArea;
...
enum ResetRequest {
    ...
    BootResetRequest, /* Boot ROM is starting -- Prepare to load Device PROM
                       */
    DevicePROMResetRequest, /* Load the next available chunk of the Device
                               PROM */
...
typedef struct {
    ...
    EmbPtr status_queue; /* Results from the host using guest's command
                            buffers */
    struct /* A word of flags ... */
    {
#if BYTE_ORDER == LITTLE_ENDIAN
        uEmbWord host_byte_order : 1; /* TRUE => byte swap data for this unit
                                         if needed */
...
enum IfNotFoundActions { CreateIfNotFound, ErrorIfNotFound };

/* Data structure passed by Lisp via the GrowDiskPartition coprocessor
 * register */
...
/* The queues contains pointers to EmbDiskQueueElement structures which
   describe a single disk I/O transaction */
...
/* The data portion of an EmbConsoleCommandEnableRunLights buffer is described
...
typedef struct {
    ...
    EmbPtr next; /* Link to next channel */
    EmbPtr outputRequestQueue; /* Commands and data to send appear in this
                                  queue */
    EmbPtr outputReplyQueue; /* Our response to the commands are placed in
                                this queue */
...
/* State of the opening display finite-state machine -- See console.c for more
 * information */
...
/* The data portion of an EmbConsoleCommandOpenDisplay buffer is described by
 * this structure */

typedef struct {
    EmbWord lastRequestNumber; /* Sequence number of last request used during
                                  open */
} EmbConsoleOpenDisplay;

/* The data portion of EmbConsoleCommandWrite and EmbConsoleCommandRead
   buffers are described by this structure */
...
/* The data portion of an EmbConsoleCommandInputWait buffer is described by
 * this structure */
...
typedef struct {
    ...
    EmbPtr guestToHostReturnQueue; /* Host to guest freed outgoing packets
                                      queue */
    EmbPtr hostToGuestSupplyQueue; /* Guest to host incoming free packets
                                      queue */
    ...
    EmbWord hardwareAddressHigh; /* Interface hardware address -- high order
                                    32 bits */
    EmbWord hardwareAddressLow; /* Interface hardware address -- low order 16
                                   bits */
    EmbWord hostPrimaryProtocol; /* Host's primary network protocol's Ethernet
                                    type */
    ...
    EmbWord nReceivedPacketsLost; /* Counter of packets received but guest
                                     wouldn't take */
...
enum EmbMessageChannelSubtype {
    ...
    EmbMessageChannelCommandSubtype, /* General operations (substitute for
                                        RPC) */
    EmbMessageChannelMBINSubtype /* Minima ROM MBIN protocol */
};

enum EmbMessageImpulse { EmbMessageImpulseNone = 0 };
...
/* Minima ROM MBIN protocol -- Unlike other message channels, the buffers in
   this channel conform to the remote memory protocol defined by the Remote
   Debugger (aka Minima Debugger), spy.c, and the various Minima ROMs */
