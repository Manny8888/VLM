...
(defmacro with-simple-binary-fixnum-operation ((a1 a2 ar t1 t2 temp1 temp2
                                                &optional inst a1-signed a2-signed)
                                               &body body)
  (check-temporaries (a1 a2 ar t1 t2)
                     (temp1 temp2))
  (let ((iolab (gensym))
        (doit (gensym)))
    `(  (label ,doit)
...
(defmacro simple-binary-immediate-arithmetic-operation (name opfn
                                                        &optional sign-extend-immp (ovflow (gensym)))
