...
(defmacro push-frame (temp temp2 temp3 temp4 temp5 &optional etag edata)
  ...
  `((LDL ,temp2 PROCESSORSTATE_CONTINUATION+4 (ivory))
    ...
    (load-constant ,temp4 %%cr.call-started "cr.call-started")
    ...
    (load-constant ,temp4 %%cr.extra-argument "cr.extra-argument")
...
(defmacro start-call-dispatch (tag data extra-tag extra-data indirect
                               temp temp2 temp3 temp6 temp7 temp8 temp9
  ...
  (check-temporaries (tag data extra-tag extra-data indirect)
                     (temp temp2 temp3 temp6 temp7 temp8 temp9))
    ...
    `((label ,again)
      ...
      (type-dispatch ,tag ,temp2 ,temp3
                     (|TypeCompiledFunction|
                      ...
                      (label ,push-extra)
                      (LDL ,temp PROCESSORSTATE_CONTROL (ivory))
                      (load-constant ,temp2 %%cr.extra-argument "cr.extra-argument")
                     ...
                     (|TypeLexicalClosure|
                      ;; (start-call-lexical-closure tag data interp extra-data extra-tag
                      ;; temp2 temp temp6 temp7 temp8 temp9 indirect)
                      ...
                      (
                       ...
                       (BR zero ,call-extra)
                       (label ,hardway))
                      (memory-read ,indirect ,tag ,data PROCESSORSTATE_DATAREAD
                                   ,temp6 ,temp7 ,temp8 ,temp9 nil t)
                      (BIS ,tag zero ,extra-tag)
                     ...
                     (:else
                      (label ,interp)
                      ;; (start-call-escape tag data notpc temp temp2 temp3
                      ;; extra-tag extra-data temp6 temp7 temp8)
                      (BIS ,tag zero ,extra-tag)
                      (BIS ,data zero ,extra-data)
...
(defmacro start-call-compiled (impctag ctag cdata temp temp2 temp3 temp4 temp5 &optional etag edata)
  (if (common-lisp:and etag edata)
      (check-temporaries (ctag cdata etag edata)
                         (temp temp2 temp3 temp4 temp5))
      (check-temporaries (ctag cdata)
                         (temp temp2 temp3 temp4 temp5)))
...
(defmacro start-call-lexical-closure (tag data interp
  ...
  (check-temporaries (tag data)
                     (temp temp2 temp3 temp4 temp5 temp6 temp7 temp8 temp9))
  `((memory-read ,data ,temp2 ,temp PROCESSORSTATE_DATAREAD
                 ,temp5 ,temp6 ,temp7 ,temp8)
    (ADDQ ,data 1 ,temp9)
    (memory-read ,temp9 ,temp4 ,temp3 PROCESSORSTATE_DATAREAD
                 ,temp5 ,temp6 ,temp7 ,temp8 nil t)
...
(defmacro start-call-escape (tag data notpc
  ...
  `((LDQ ,temp2 PROCESSORSTATE_TRAPVECBASE (ivory))
    ...
    (TagType ,tag ,tag)
    (ADDQ ,tag ,temp ,temp)
    (memory-read ,temp ,temp4 ,temp3 PROCESSORSTATE_DATAREAD
                 ,temp5 ,temp6 ,temp7 ,temp8 nil t)
...
(defmacro finish-call-guts (nargs disp applyp cr nfp
                            temp temp2 temp3 temp4 temp5)
  (check-temporaries (nargs disp applyp cr nfp 'arg2)
                     (temp temp2 temp3 temp4 temp5))
  (let ()
    `((SRL ,applyp #.(- 10 3) ,applyp)
      ...
      ;; ;; (comment "Update CP")
      (load-constant ,temp5 %%cr.call-trace "cr.call-trace")
...
(defmacro push-apply-args (min max temp temp2 temp3 &optional cr)
  (if cr
      (check-temporaries (min max cr)
                         (temp temp2 temp3))
      (check-temporaries (min max)
                         (temp temp2 temp3)))
  (let ((l1 (gensym))
        (ent (gensym)))
  ...
  (let ((done (or done-label (gensym)))
    ...
    `((stack-top2 ,tag ,data)
      (type-dispatch ,tag ,temp ,temp2
                     ...
                     (|TypeNIL|
                      (get-control-register ,temp3 "Get the control register")
                      (load-constant ,temp4 %%cr.apply "cr.apply")
...
(defmacro cleanup-frame (cr done-label
  ...
  (let ((reallydone (or done-label (gensym)))
    ...
    `(
      (label ,top)
      (load-constant ,temp %%cr.cleanup-catch "cr.cleanup-catch")
      ...
      (load-constant ,temp3 %%cr.cleanup-catch "cr.cleanup-catch")
      ...
      (BR zero ,top)
      (label ,almostdone)
      (load-constant ,temp %%cr.cleanup-bindings "cr.cleanup-bindings")
      (AND ,temp ,cr ,temp2)
      ...
      (label ,done)
      (load-constant ,temp3 %%cr.trap-on-exit-bit "cr.trap-on-exit-bit")
      (AND ,temp3 ,cr ,temp2)
...
(defmacro do-unwind-protect (cr temp temp2 temp3 temp4 temp5 temp6
  ...
  (let ((pushpc (gensym))
    ...
    `((LDL ,temp4 PROCESSORSTATE_CATCHBLOCK (ivory))
      ...
      (load-constant ,temp %%cr.cleanup-in-progress "cr.cleanup-in-progress")
      (BIS ,cr ,temp ,cr)
...
(defmacro abandon-frame-simple (restorepctest cr cleanuplabel
                                temp temp2 temp3 temp4 temp5 temp6 next-cp)
    ...
    `((Comment "Restore machine state from frame header.")
      ,@(let ((saved-continuation-tag temp2)
          ...
          `((LDL ,saved-continuation-data 0 (iFP))
            (load-constant ,temp %%cr.cleanup-bits "cleanup bits")
      ...
      (load-constant ,temp2 %%cr.trace-pending "cr.trace-pending")
      (AND ,temp2 ,cr ,temp2)
