 ...
 * aistat.s for the ASM interpreter.  The asm interpreter caches much of its
 * state in alpha registers while running, so it is important for the
 * interpreter to be halted before anyone 'dinks' with the state. The
 * interpreter encaches the state when it states up and decaches the state
 * when it halts.  A couple of special exceptions are please-stop and runningp
 * which are not cached, and are used to request the interpreter to stop, and
 * test whether it has stopped or not.
 */
...
char *haltreason(int reason)
{
    ...
    case HaltReason_FatalStackOverflow:
        return "STACK OVERFLOW NOT IN EMULATOR MODE";
    case HaltReason_IllegalTrapVector:
        return "NON-PC IN TRAP VECTOR";
    default:
...
void CheckMat()
{
    ...
    for (i = 0, matline = debugcopymat; i < 13; i++, matline += 64) {
        ...
        for (e = &MemoryActionTable[i][0], j = 0; e < &MemoryActionTable[i][64]; e++, j++) {
            if (matline[j] != *e) {
                LogMessage("CheckMat", "MAT difference found at [i=%d,j=%d] MAT=%d copymat=%d\n", i, j, *e, matline[j]);
...
void InitializeIvoryProcessor(Integer *basedata, Tag *basetag)
{
     ...
     * processorstate. We also allocate the stack-cache in this block, so that
     * stack-cache and processor-state references do not collide
     */
    if (processor == NULL) {
            ...
            *tablePointer = (char *)matline; /* (char*)&MemoryActionTable[i][0];*/
            ...
            for (e = &MemoryActionTable[i][0], j = 0; e < &MemoryActionTable[i][64]; e++, j++) {
                *e &= ~MemoryAction_Transport;
                if (*e) {
                    // accumulate mask of types with action
                    mask |= (1L << j);
                }
                // copy bits into copymat
                matline[j] = *e;
            }
            *maskPointer = mask;
        }
...
int Runningp(void) { return processor->runningp; }
