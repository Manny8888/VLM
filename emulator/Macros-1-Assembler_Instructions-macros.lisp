...
(defmacro define-procedure (name (&rest args) &body body &environment env)
  (let ((*function-being-processed* name))
    `((start ,name ,(length args))
      (label ,name)
      ,@(grimoire-assembler:collecting-function-epilogue body env)
      (end ,name))))
...
(defmacro define-subroutine (name (&rest args)
                             (linkage &rest temps-to-save)
                             &body body
                             &environment env)
  ...
  (let ((*function-being-processed* name)
    ...
    `((start ,name ,(length args))
      ...
      ,@(grimoire-assembler:collecting-function-epilogue
         `(,@body
           ,@(loop for offset upfrom 8 by 8
                   for temp in temps-to-save
...
(defmacro define-fast-subroutine (name (&rest args)
                                  (linkage &rest temps-to-save)
                                  &body body
                                  &environment env)
...
(defmacro load-constant (reg constant &optional comment)
  ...
  (let* ((low (dpb constant (byte 16 0) (- (ldb (byte 1 15) constant))))
    ...
    (assert (zerop (ldb (byte 16 0) high)) () "Don't know how to load ~D" constant)
    `(,@(cond
          ((zerop constant) `((BIS zero zero ,reg)))
          ((zerop high)     `((LDA ,reg ,low (zero))))
          ((zerop low)      `((LDAH ,reg ,(ash high -16) (zero))))
          (t `((LDA ,reg ,low (zero))
