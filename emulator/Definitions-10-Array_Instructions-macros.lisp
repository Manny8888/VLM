          ...
          (let* ((bias )
                 (tests
                   (loop repeat (1+ (length keys))
                         as try =
                                (loop for (key . rest) on keys with candidates =
                                      `((,#'(lambda (k) (< k 0))
...
(defmacro recompute-array-register (address inst control base length done-label
                                    temp temp2 temp3 temp4 temp5
                                    &optional (error 'array-register-format-error))
  "Leaves control, base, and length in the named args"
  (check-temporaries (address control base length)
                     (temp temp2 temp3 temp4 temp5))
  (let ((iop (gensym))
        (reallyexc (gensym))
        (iex (gensym))
...
(defmacro setup-long-array-register (atag adata temp force1d temp3 temp4 temp5 temp6 temp7 temp8
                                     temp9 temp10 temp11 temp12 temp13 temp14 temp15 temp16)
  (check-temporaries (atag adata)
                     (temp force1d temp3 temp4 temp5 temp6 temp7 temp8 temp9 temp10
                           temp11 temp12 temp13 temp14 temp15 temp16))
  (let ((temp2 force1d)
        (iex (gensym))
        (leafarray (gensym))
                     ...
                     (|TypeArray|
                      ...
                      ,@(let ((bpd adata)
                          ...
                          `((SRL ,temp3 |ArrayBytePackingPos| ,bp)	; byte-packing
                            ...
                            (memory-read ,indirect ,temp6 ,temp4 PROCESSORSTATE_HEADER
                                         ,temp7 ,temp8 ,temp10 ,temp11)
                            ;;+++ check header?
