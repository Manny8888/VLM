...
(defmacro cdr-internal (tag data opcode vma temp3 temp4 temp5 temp6 &optional signedp)
  ...
  (let ((readcdr (gensym))
    ...
    `(;; Allows arg-fetch to be signed
      ...
      (type-dispatch ,tag ,temp3 ,temp4
                     (|TypeList|
                      (memory-read ,vma ,tag ,data PROCESSORSTATE_CDR
                      ...
                      (cdr-code-dispatch
                       ...
                       (|CdrNormal|
                        ...
                        (label ,readcdr)
                        (memory-read ,vma ,tag ,data PROCESSORSTATE_DATAREAD
                                     ,temp3 ,temp4 ,temp5 ,temp6 ,endcdr ,signedp)
                        )
                       (|CdrNil|
                        (get-nil2 ,tag ,data)
                        (BR zero ,endcdr))
...
(defmacro carcdr-internal (tag data dtag ddata opcode vma temp3 temp4 temp5 temp6
  ...
  (let ((forwarded (gensym))
    ...
    `(;; Allows arg-fetch to be signed
      ...
      (type-dispatch ,tag ,temp3 ,temp4
                     (|TypeList|
                      (memory-read ,vma ,dtag ,ddata PROCESSORSTATE_DATAREAD
                                   ,temp3 ,temp4 ,temp5 ,temp6 nil ,signedp)
                      (SUBL ,vma ,data ,temp3)
                      ...
                      (cdr-code-dispatch
                       ...
                       (|CdrNormal|
                        (ADDQ ,vma 1 ,vma)
                        (memory-read ,vma ,dtag ,ddata PROCESSORSTATE_DATAREAD
                                     ,temp3 ,temp4 ,temp5 ,temp6 ,end-carcdr ,signedp)
                        )
                       (|CdrNil|
                        (get-nil2 ,dtag ,ddata)
                        (BR zero ,end-carcdr))
                     ...
                     (:else
                      ...
                      (BIS ,dtag zero ,tag)
                      (BIS ,ddata zero ,data)
                      (memory-read ,vma ,dtag ,ddata PROCESSORSTATE_CDR
                                   ,temp3 ,temp4 ,temp5 ,temp6 nil t)
                      (BR zero ,cdr-ed)))
      (label ,end-carcdr))))
...
(defmacro isettocdrpushcar (poperand tag data dtag ddata vma
                            temp2 temp9 temp3 temp4 temp5 temp6 temp7 temp8)
  "DTAG and DDATA should be the canonical tag/data registers"
  (declare (ignore temp2 temp3 temp4))
  (check-temporaries (poperand tag data)
                     (vma temp5 temp6 temp7 temp8 temp9))
  (let ((loclab (gensym)))
    `((stack-read2 ,poperand ,tag ,data "Get the operand from the stack.")
      ...
      (BEQ ,temp5 ,loclab)
      (carcdr-internal ,tag ,data ,dtag ,ddata set-to-cdr-push-car ,vma
                       ,temp5 ,temp6 ,temp7 ,temp8 t)
      (TagType ,dtag ,dtag)
...
(defmacro carcdrloop ((instruction  obj-tag obj-data car-tag car-data cdr-tag cdr-data
  ...
  (let ((break (gensym))
    ...
    `(with-multiple-memory-reads (,temp9 ,temp10 ,temp11 ,temp12)
       ...
       ,@(ecase loopstep
           (CDR `(#+list-inline
                  (cdr-internal ,cdr-tag ,cdr-data ,instruction ,vma
                                ,temp5 ,temp6 ,temp7 ,temp8)	;cddr of init
                  #-list-inline
                  (BSR r0 |CdrInternal|)))
           ((NIL) ()))
       ...
       (BEQ ,temp5 ,end)
       #+list-inline (carcdr-internal ,car-tag ,car-data ,cdr-tag ,cdr-data
                                      ,instruction ,vma ,temp5 ,temp6 ,temp7 ,temp8)
