...
(defun initialize-memory-actions ()
  (let ((pointer-data-types
          (list
           dtp-double-float dtp-bignum dtp-big-ratio dtp-complex dtp-spare-number dtp-instance
           dtp-list-instance dtp-array-instance dtp-string-instance dtp-nil dtp-list
           dtp-array dtp-string dtp-symbol dtp-locative dtp-lexical-closure
           dtp-dynamic-closure dtp-compiled-function dtp-generic-function dtp-spare-pointer-1
           dtp-spare-pointer-2 dtp-bound-location dtp-logic-variable dtp-even-pc dtp-odd-pc
           dtp-call-compiled-even dtp-call-compiled-odd dtp-call-indirect dtp-call-generic
        ...
        (immediate-data-types
          (list
           dtp-fixnum dtp-small-ratio dtp-single-float dtp-physical-address
           dtp-spare-immediate-1 dtp-character
    ...
    (loop for cycle-actions in *memory-actions-table* do
      (destructuring-bind (cycle-type data null head hfwd efwd 1fwd evcp gc mon ptr bl lv)
        ...
        (labels ((translate-symbolic-action (symbol)
          ...
          (initialize-cycle null +dtp-null+)
          (initialize-cycle head dtp-header-i dtp-header-p)
          (initialize-cycle hfwd dtp-header-forward)
          (initialize-cycle efwd dtp-element-forward)
          (initialize-cycle 1fwd dtp-one-q-forward)
          (initialize-cycle evcp dtp-external-value-cell-pointer)
          (initialize-cycle gc dtp-gc-forward)
          (initialize-cycle mon dtp-monitor-forward)
          (initialize-cycle ptr
                            ...
                            dtp-monitor-forward)
          (initialize-cycle bl dtp-bound-location)
          (initialize-cycle lv dtp-logic-variable))))))
...
(defmacro with-multiple-memory-reads ((vmdata vmtags base limit
  ...
  (let ((*memoized-vmdata* vmdata)
        ...
        (*memoized-base* (common-lisp:and (not cant-be-in-cache-p) base))
        (*memoized-limit* (common-lisp:and (not cant-be-in-cache-p) limit))
  ...
  (let* ((args (list vma tag data))
    ...
    `(define-fast-subroutine ,name () (,linkage)
       (using-multiple-memory-reads (,vmdata ,vmtags ,base ,limit)
                                    (memory-read ,vma ,tag ,data ,cycle
                                                 ,temp ,temp2 ,temp3 ,temp4 nil nil t)))))
  ...
  (let () #+ign ((args (map 'list #'real-reg args))
    ...
    (loop with bname and bargs and blinkage and bmerit
          for ((targs tcycle ttemps tcaches) name linkage) in *memory-subroutines* do
            (when (common-lisp:and (equal cycle tcycle)
                            (equal temps ttemps)
...
;;; temp4)
(defun memory-read-internal (vma tag data cycle
                             temp temp2 temp3
                             &optional temp4 done-label signedp inlinep
                             &aux subr args linkage)
  ...
  (let* ((cycle-number (case cycle
    ...
    (flet ((main-expansion ()
             `(;; ;; (comment "Memory Read Internal")
               ...
               ,@(if (common-lisp:and (eq cycle :general) (or temp4 *cant-be-in-cache-p*))
        ...
        (unless (eq cycle 'processorstate_raw)
          (push
           `(
             ...
             ,@(when (common-lisp:and canindirect (not cycle-indirect-mask))
                 `((label ,checkindirect)
...
(defmacro memory-read (vma tag data cycle
                       temp temp2 temp3 temp4
                       &optional done-label signedp inlinep)
  (check-temporaries (vma)
                     (tag data temp temp temp2 temp3 temp4))
  (assert (common-lisp:and (not (eql tag 'zero)) (not (eql data 'zero))))
  `(,@(memory-read-internal vma tag data cycle
                            temp temp2 temp3 temp4 done-label signedp inlinep)))

(defmacro memory-write (vma tag data cycle
                        temp temp2 temp3 temp4
...
(defmacro store-contents (vma new-tag new-data cycle tag data
                          temp temp2 temp3
                          &optional temp4 done-label)
  (if temp4
      (check-temporaries (vma new-tag new-data)
                         (tag data temp temp2 temp3 temp4))
      (check-temporaries (vma new-tag new-data)
                         (tag data temp temp2 temp3)))
  (assert (common-lisp:and (not (eql new-tag 'zero)) (not (eql new-data 'zero))))
  `(,@(memory-read-internal vma tag data cycle temp temp2 temp3 temp4 nil t)
    ;; ;; (comment "Merge cdr-code")
    (AND ,new-tag #x3F ,data)
    (AND ,tag #xC0 ,tag)
    (BIS ,tag ,data ,tag)
    (memory-write ,vma ,tag ,new-data PROCESSORSTATE_RAW
                  ,temp ,temp2 ,temp3 ,data ,temp4 ,done-label)))
...
(defmacro store-conditional-internal (vma oldtag olddata newtag newdata faillab
  ...
  (let (;; readability
    ...
    `(;; ;; (comment "Read the location, checking write access")
      ...
      ;; Update the object
      (memory-write ,vma ,tag ,newdata PROCESSORSTATE_RAW
                    ,temp ,temp2 ,temp3 ,temp5 ,temp6 ,done-label))))
