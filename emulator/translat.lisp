...
(defclass fullword-instruction (ivory-instruction)
  ((constant :initarg :constant)))


(defclass alpha-native-instruction (fullword-instruction)
  ())


(defclass alpha-native-branch-instruction (alpha-native-instruction)
  ((target :initarg :target :initform nil)))


(defclass halfword-instruction (ivory-instruction)
  ((operand :initform 0 :initarg :operand)))


(defclass ibranch-format-instruction (halfword-instruction)
  ((target :initarg :target :initform nil)))
...
(defmethod instruction-labeledp ((decinst ivory-instruction))
  (with-slots (label) decinst
    label))


(defmethod label-instruction ((decinst ivory-instruction))
  (with-slots (label) decinst
    (first
     (if label
         label
...
(defmethod instruction-kind ((decinst fullword-instruction)) :fullword)


(defmethod instruction-kind ((decinst halfword-instruction)) :halfword)


(defmethod entry-instructionp ((decinst fullword-instruction)) ())


(defmethod entry-instructionp ((decinst halfword-instruction))
  (with-slots (opcode) decinst
    (or (= opcode #o176) (= opcode #o177))))


(defmethod native-instructionp ((decinst fullword-instruction))
  (with-slots (opcode) decinst
    (= opcode i-lisp-compiler:*vlm-native-instruction-opcode*)))


(defmethod native-instructionp ((decinst halfword-instruction))
  ())
...
(defmethod instruction-name ((decinst fullword-instruction))
  (with-slots (opcode) decinst
    (svref *fullwordinstns* opcode)))


(defmethod instruction-name ((decinst halfword-instruction))
  (with-slots (opcode) decinst
    (svref *halfwordinstns* opcode)))


;; Broken out for calling from Genera disassembler
#+T  (defun i-lisp-compiler:vlm-print-native-instruction (constant stream &optional pc)
...
(defmethod print-instruction ((decinst fullword-instruction) &optional (stream t))
  (with-slots (pc constant nextpc) decinst
    (fresh-line stream)
    (if (native-instructionp decinst)
        (with-slots (constant) decinst
...
(defmethod print-instruction ((decinst halfword-instruction) &optional (stream t))
  (with-slots (pc operand nextpc) decinst
    (fresh-line stream)
    (let* ((instname (instruction-name decinst)))
      (future-common-lisp::format stream "~o: ~a operand=~a nextpc=~o~%"
                                  pc instname operand nextpc))))

(defmethod print-function ((decfcn decoded-function) &optional (stream t))
  (with-slots (instructions) decfcn
    (dolist (inst instructions)
      (print-instruction inst stream))))

(defmethod print-function ((ts translation-state) &optional (stream t))
  (with-slots (source target) ts
...
(defmethod findpc ((decfcn decoded-function) apc)
  (with-slots (instructions) decfcn
    (dolist (in instructions)
      (with-slots (pc) in
        (if (= pc apc) (return-from findpc in))))))

(defmethod abstract-branch-target ((decfcn decoded-function)
                                   (inst ibranch-format-instruction))
  (with-slots (instruction) decfcn
    (with-slots (pc operand target) inst
...
(defmethod linkup-function ((decfcn decoded-function))
  (with-slots (instructions) decfcn
    ...
    (dolist (instn instructions)
      ...
      (with-slots (nextinstn nextpc pc) instn
    ...
    (do ((iorder 0 (+ iorder 1))
         (instn (first instructions) (with-slots (nextinstn) instn nextinstn)))
        ((null instn)
      ...
      (with-slots (order) instn
        (setq order iorder)))))

#+T (defun i-lisp-compiler:vlm-decode-ivory-function (fcn)
  (assert (typep fcn 'compiled-function))
  (let ((decfcn (make-instance 'decoded-function))
        (endcc nil)
        (info nil))
    (do ((index 0 (+ index 1)))
        (endcc ())
    ...
    (with-slots (packed-instructions debug-info instructions) decfcn
...
(defun make-hwinst (cc pc opcode datum tag nextpc)
  (make-instance (if (branch-format-hw-opcodep opcode)
...
(defun make-fwinst (cc pc opcode datum tag nextpc)
  (make-instance 'fullword-instruction
                 :sequence cc
                 :pc pc
                 :nextpc (+ (ash nextpc 1) (if (eq tag si:dtp-odd-pc) 1 0))
                 :opcode opcode
...
(defmethod copy-instruction ((oldinst fullword-instruction))
  (with-slots (label sequence pc nextpc opcode constant) oldinst
    (make-instance 'fullword-instruction
                   :label label
                   :sequence sequence
                   :pc pc
...
(defmethod copy-instruction ((oldinst halfword-instruction))
  (with-slots (label sequence pc nextpc opcode operand) oldinst
    (make-instance 'halfword-instruction
                   :label label
                   :sequence sequence
                   :pc pc
                   :nextpc nextpc
...
(defmethod copy-instruction ((oldinst ibranch-format-instruction))
  (with-slots (label sequence pc nextpc opcode operand target) oldinst
    (make-instance 'ibranch-format-instruction
                   :label label
                   :sequence sequence
                   :pc pc
                   :nextpc nextpc
...
(defun make-alpha-instruction (bits)
  (let* ((fnbits (if (zerop (ldb (byte 1 31) bits))
    ...
    (make-instance (if (eq opcodetype 'br)
...
(defmethod add-instruction ((nuinst ivory-instruction) (istream translation-state))
  (with-slots (target pendinglabel) istream
    (with-slots (instructions icount) target
      (with-slots (order label) nuinst
        (when pendinglabel
...
(defmethod decode-ivory-instruction ((decfcn decoded-function) pc tag data word)
  (multiple-value-bind (cc even-tag even-pc odd-tag odd-pc)
    ...
    (with-slots (instructions packed-instructions) decfcn
...
(defmethod set-instn-cdr-code ((decinst ivory-instruction) cc)
  (with-slots (sequence) decinst
    (setq sequence cc)))
...
(defmethod setlabel ((istream translation-state) label)
  (with-slots (pendinglabel) istream
    (if (listp label)
...
i#+T (defmethod i-lisp-compiler:vlm-translate-function ((decfcn decoded-function))
  (let* ((newfcn (make-instance 'decoded-function))
         (tstate (make-instance 'translation-state :source decfcn :target newfcn))
         (newinfo nil)
         (fcnloc nil))
    (with-slots (instructions debug-info) decfcn
      (setq newinfo debug-info)
      (let ((*function-epilogue* nil))
        (dolist (instn instructions)
          (with-slots (label) instn
          ...
          (with-slots (pc) instn
            (with-slots (target) tstate
              (with-slots (instructions) target
                (with-slots (originalpc) (first instructions)
                  (setf originalpc pc))))))
        (loop while *function-epilogue*
              do (assemble-asm-form (shiftf *function-epilogue* nil) tstate))))
    (emit-escape-blocks tstate)
    (with-slots (instructions debug-info) newfcn
    ...
    (with-slots (instructions debug-info) newfcn
      (let ((vca (cdr (assoc :variable-creation-alist (cdr debug-info)))))
        (loop for inst in instructions
              while vca do
                (with-slots (originalpc pc) inst
                  (let ((hit (assoc originalpc vca)))
                    (when hit
                      (setf (car hit) pc)
                      (pop vca)))))))
    (with-slots (target i-lisp-compiler:newfun) tstate
      (with-slots (packed-instructions packed-size debug-info) target
...
(defmethod pack-instructions ((decfcn decoded-function))	;
  (with-slots (instructions packed-instructions packed-size) decfcn
    (let* ((thispc 0)				;The current PC
      ...
      (dolist (instn instructions)
        ...
        (with-slots (pc opcode nextpc nextinstn label) instn
      ...
      (dolist (ob alpha-branches-list)
        (with-slots (constant target pc) ob
          (let* ((targetinst (assoc target labels-alist))
                 (sourcepc pc))
            (if (null targetinst)
        ...
        (let* ((inst (car ilist))
          ...
          (cond
            ((eq ityp :fullword)		; Simple case. 1 fullword instruction
             (with-slots (opcode constant sequence pc) inst
            ...
            ((eq ityp :halfword)
             (cond ((lisp:and (eq nextityp :halfword) (not (entry-instructionp inst)))
                    (pop ilist)
                    (with-slots (operand opcode) nextinst
                      (setq opcode2 opcode)
                      (setq hwinstn2 operand))))
             (with-slots (tag opcode operand sequence) inst
...
(defmethod set-instruction-mode ((ts translation-state) mode)
  (assert (or (eq mode :emulated) (eq mode :native)))
  (with-slots (nativemodep) ts
    (cond
...
(defclass escape-block ()
  ((instruction :initarg :instruction)
   (returnto :initarg :returnto)
   (tosstatus :initarg :tosstatus)))

(defmethod emit-make-escape ((istream translation-state) instn &optional tosstatus)
  (with-slots (target) istream
    (with-slots (escapeblocks) target
      (let* ((escapelabel (gensym))
        ...
        (with-slots (label) nuinst
          (setf label `(,escapelabel)))
        (push (make-instance 'escape-block
                             :instruction nuinst
...
(defmethod emit-escape-blocks ((istream translation-state))
  (with-slots (target) istream
    (with-slots (escapeblocks) target
...
(defmethod emit-block ((istream translation-state) (esc escape-block))
  (with-slots (instruction returnto tosstatus) esc
    (with-slots (label) instruction
    ...
    (ecase tosstatus
      ((nil :arg6)
       (let ((returninst (make-instance 'ibranch-format-instruction
                                        :opcode #o0174	; +++ Branch
                                        :target returnto)))
...
(defmethod emit-compute-operand-value ((istream translation-state)
                                       instn dest &rest stack-options)
  (with-slots (operand) instn
    (decode-operand-specifier
     (relto offset popp) operand
...
(defmethod emit-compute-operand-value2 ((istream translation-state)
...
(defmethod emit-compute-operand-data ((istream translation-state)
                                      instn dest &rest stack-options)
  (with-slots (operand) instn
    (decode-operand-specifier
     (relto offset popp) operand
...
(defmethod emit-compute-operand-address ((istream translation-state)
                                         instn dest)
  (with-slots (operand) instn
    (decode-operand-specifier
     (relto offset popp) operand
...
(defmethod emit-compute-operand-register-offset  ((istream translation-state)
                                                  instn dest)
...
(defmacro TOSvalid (status &optional cdr)
  `(with-slots (toscache toscdr) istream
...
(defmethod allocate-register ((ts translation-state))
  (with-slots (freeregs) ts
    (if (null freeregs) (error "Not enough registers!"))
    (let ((reg (car (last freeregs))))
      (setf freeregs (delete reg freeregs))
      reg)))

(defmethod free-register ((ts translation-state) reg)
  (with-slots (freeregs) ts
    (pushnew reg freeregs)))
...
(defmacro def-fullword-translation (name arglist &rest body)
  `(defmethod ,name ((istream translation-state) ,@arglist) ,@body))
...
(defmethod passthrough-ivory-instruction ((istream translation-state) instn)
