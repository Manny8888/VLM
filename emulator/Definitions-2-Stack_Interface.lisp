...
(defmacro stack-fill (VMA SCA count
                      temp temp2 temp3 temp4)
  (check-temporaries (VMA SCA count)
                     (temp temp2))
  (let ((l1 (gensym))
        (l2 (gensym)))
...
(defmacro stack-cache-overflow-check (temp temp2 temp3 temp4 temp5
  ...
  (check-temporaries (sp handler-arg)
...
(defmacro stack-block-copy (from to count ccp upp temp temp2)
  (check-temporaries (from to count)
                     (temp temp2))
  (let ((l1 (gensym))
        (l2 (gensym)))
    `(,@(when ccp
